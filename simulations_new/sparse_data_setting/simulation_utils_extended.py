...
def generate_data(n, m, p, func, shared_gp=True, random_state=1, rho=0.1, sigma2_2=1, return_rand_effects=False, no_rand_intercept=False):
    # Features
    ...
    if no_rand_intercept:
        G = np.concatenate(GP_list) + 0.005 * np.concatenate(times_arrays)
...
def generate_datasets(n, m, p, n_datasets, n_valid, func, shared_gp=True, random_state=1, include_cat_feature=False, rho=0.1, sigma2_2=1, stan_GP=False, no_rand_intercept=False):
    ...
    for i in range(n_datasets):
        data, X, y, F = generate_data(n, m, p, func, shared_gp, random_state=i+random_state, rho=rho, sigma2_2=sigma2_2, no_rand_intercept=no_rand_intercept)
    ...
    for i in range(n_valid):
        data, X, y, _ = generate_data(n, m, p, func, shared_gp, random_state=i+n_datasets+random_state, rho=rho, sigma2_2=sigma2_2, no_rand_intercept=no_rand_intercept)
